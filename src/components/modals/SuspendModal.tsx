...
export default function SuspendModal({
  ...
  const handleQuickSelect = (days: number) => {
    ...
    setError(''); // Clear error when quick select is used
  };
    ...
    <>
            ...
            <div className="space-y-4">
              ...
              <div className="relative space-y-2">
                ...
                <Input
                  ...
                  className="border-zinc-600 bg-zinc-800 text-white"
                />
                {error && <p className="text-red-reject text-sm">{error}</p>}
              </div>
      ...
      <Dialog open={showErrorModal} onOpenChange={setShowErrorModal}>
        <DialogContent className="bg-background border-zinc-700 text-white sm:max-w-md">
          ...
          <DialogDescription className="sr-only">Validation error</DialogDescription>

          <div className="flex flex-col space-y-6 p-6">
            <div className="flex justify-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-red-reject">
                <AlertTriangle className="h-8 w-8 text-red-reject" />
              </div>
            </div>

            <div className="text-center">
              <h3 className="text-xl font-semibold text-white">Validation Error</h3>
              <p className="text-lighter-gray-text mt-2">{errorMessage}</p>
            </div>
