  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="bg-background border-zinc-700 text-white sm:max-w-md">
        ...
        <DialogTitle className="sr-only">Unban {entityType}</DialogTitle>
        <DialogDescription className="sr-only">Confirm unban for {entityName}</DialogDescription>

        <div className="flex flex-col space-y-6 p-8">
          {/* Icon */}
          <div className="flex justify-center">
            <div className="border-primary-green flex h-16 w-16 items-center justify-center rounded-full border-2">
              <ShieldCheck className="text-primary-green h-8 w-8" />
            </div>
          </div>

          {/* Title */}
          <div className="text-center">
            <h3 className="text-2xl font-semibold text-white">Unban {entityType}</h3>
          </div>

          {/* Message with icon */}
          <div className="flex items-center justify-center gap-2">
            <ShieldCheck className="text-primary-green h-4 w-4" />
            <p className="text-primary-green">Restore full access</p>
          </div>
          ...
          <div className="text-lighter-gray-text text-sm leading-relaxed">
            You are about to unban <span className="font-medium text-white">{entityName}</span>.
            <br />
            <br />
            This will immediately restore full access and remove all restrictions from the{' '}
            {entityType.toLowerCase()}. They will be able to use the platform normally again.
            <br />
            <br />
            Do you want to proceed with unbanning?
          </div>
